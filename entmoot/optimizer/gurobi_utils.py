...
def add_std_to_gurobi_model(est, model):
    ...
    -
    """
    print("Now add Gurobi model")
    est.std_estimator.add_to_gurobi_model(model)
    model.update()

def set_gurobi_init_to_ref(X, est, model):
    ...
    -
    """
    ref_points_unscaled = np.array(X)
...
def add_acq_to_gurobi_model(model, est, acq_func="LCB", acq_func_kwargs=None):
    ...
    if est.std_estimator:
        ...
        mu = get_gurobi_obj(
            model, est, return_std=False, acq_func_kwargs=acq_func_kwargs
        )
        ob_expr = mu
