...
python src/train_scripts/train_celeba_pixelsnail.py \
  --root_dir="$root_dir/celeba/pixelsnail_bottom" \
  --seed="$seed" $gpu \
  --shape=16 \
  ...
  --n_block=4 \
  --hier="bottom" \
  --load_from_checkpoint="$root_dir/celeba/pixelsnail_bottom/lightning_logs/version_0/checkpoints/epoch=29-step=66359.ckpt"
