""" Trains a VQ-VAE for the CelebA task """

import argparse
import pytorch_lightning as pl

from weighted_retraining.celeba.celeba_vqvae_256 import CelebaVQVAE
...
def main(args):
    # Load model
    model = CelebaVQVAE(args)
    if hparams.load_from_checkpoint is not None:
        model = CelebaVQVAE.load_from_checkpoint(hparams.load_from_checkpoint)
...
if __name__ == "__main__":
    ...
    parser = CelebaVQVAE.add_model_specific_args(parser)
    ...
    utils.add_default_trainer_args(parser, default_root="logs/train/CelebaVQVAE")
