""" Run weighted retraining for CelebA with the optimal model """
...
# My imports
from weighted_retraining.dataloader_celeba_weighting import *
from weighted_retraining.temperature_scaling import *
from weighted_retraining.resnet50 import resnet50
from weighted_retraining.utils import output_to_label
from weighted_retraining import utils
from weighted_retraining.opt_scripts import base as wr_base
from weighted_retraining.torch_mimicry.training.trainer import Trainer
from weighted_retraining.torch_mimicry.nets.sngan.sngan_64 import SNGANGenerator64, SNGANDiscriminator64
from weighted_retraining.dataloader_celeba_weighting import CelebaWeightedTensorDataset
from weighted_retraining import GP_TRAIN_FILE, GP_OPT_FILE, DNGO_TRAIN_FILE, GP_OPT_SAMPLING_FILE
...
def generate_samples(netG, netD, n_samples, discriminator_quantile_threshold, device):
    batch_size = 50
    print_every = 10 # TODO: Delete at some time
    ...
    if discriminator_quantile_threshold != 0.0:
        with torch.no_grad():
            ...
            for idx in range(0, n_samples, batch_size):
                ...
                #     end_time = time.time()
                #     print(
                #         "INFO: Dicriminator probabilities {}/{} [Random Seed {}] ({:.4f} sec/idx)"
                #             .format(
            ...
            #threshold_quantile = np.quantile(probabilities, 1 - keep_fraction)
...
def latent_optimization(args, netG, netD, scaled_predictor, dataloader, num_queries_to_do, bo_data_file, bo_run_folder, device, pbar=None, postfix=None, curr_generator_file=None, curr_discriminator_file=None):
    """ do latent space optimization with Bayesian Optimization """

    ##################################################
    # Prepare BO
    ##################################################

    # First, choose points to train surrogate model on!
    # As a SN-GAN is not capable to simply encode the traning data to the latent space, the procedure has to be adapted:
    # First, N (default: 100,000) samples are generated by the generator, then, optionally, only a fraction of the samples according
    # to the dicriminator probability prediction (whether sample is "real") is kept.
