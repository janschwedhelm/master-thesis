...
from weighted_retraining import GP_TRAIN_FILE, GP_OPT_FILE, DNGO_TRAIN_FILE
...
# def latent_sampling(args, model, datamodule, num_queries_to_do, sampling_method="normal", get_props=True,
#                     filter_unique=True, pbar=None):
...
#         elif sampling_method == "gmm":
#             if not args.n_gmm_components:
#                 raise Exception("Please specify number of components to use for the GMM model.")
#             z_train = model.encode_to_latent_space(torch.tensor(datamodule.data_train[-500:]).unsqueeze(1))
#             gmm = GaussianMixture(n_components=args.n_gmm_components, random_state=args.seed).fit(z_train.detach().numpy())
#             z_sample = torch.tensor(gmm.sample(num_queries_to_do)[0], dtype=torch.float32)
#         else:
...
#             gm = GaussianMixture(n_components=1, random_state=args.seed).fit(z_train.detach().numpy())
#             z_sample = torch.tensor(gm.sample(num_queries_to_do)[0], dtype=torch.float32)
#         elif sampling_method == "gmm":
#             if not args.n_gmm_components:
#                 raise Exception("Please specify number of components to use for the GMM model.")
#             z_train = model.encode_to_latent_space(torch.tensor(datamodule.data_train[-500:]).unsqueeze(1))
...
#             raise NotImplementedError(sampling_method)
#     else:
#         raise NotImplementedError(args.pretrained_model_file)
#
#     return _batch_decode_z_and_props(model, z_sample, args, get_props=get_props, filter_unique=filter_unique)
