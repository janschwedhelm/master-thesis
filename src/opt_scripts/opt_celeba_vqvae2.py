...
from src.celeba.celeba_vqvae2 import CelebaVQVAE2
...
from src import ENTMOOT_TRAIN_OPT_FILE
...
def _encode_images(model, dataset):
    ...
    for image_tensor_batch in dataloader:
        with torch.no_grad():
            _, _, _, id_t, id_b = model.encode(image_tensor_batch[0].to(model.device))
            id_t = id_t.reshape(-1, id_t.shape[1]**2).cpu().numpy()
            id_b = id_b.reshape(-1, id_b.shape[1] ** 2).cpu().numpy()
            z_encode_t.append(id_t)
            z_encode_b.append(id_b)
            del id_t, id_b

    # Aggregate array
    z_encode_t = np.concatenate(z_encode_t, axis=0)
    z_encode_b = np.concatenate(z_encode_b, axis=0)
    return z_encode_t, z_encode_b


def _batch_decode_z_and_props(model, predictor, z_t, z_b, get_props=True):
    """
    ...
    z_decode_upscaled = []
    batch_size = 1000
    for j in range(0, len(z_t), batch_size):
        with torch.no_grad():
            img = model.decode_code(z_t[j: j + batch_size], z_b[j: j + batch_size])
            # img = img.cpu().numpy()
...
def latent_optimization(args, model, scaled_predictor, pixelsnail_bottom,datamodule, num_queries_to_do, bo_data_file, bo_run_folder, curr_model_path, pbar=None, postfix=None):
    ...
    if args.gpu:
        model = model.cuda()
    latent_points_t, latent_points_b = _encode_images(model, temp_dataset)
    ...
    def _save_bo_data(latent_points, targets):
        ...
        np.savez_compressed(
        bo_data_file,
            X_train=latent_points.astype(np.int64),
            X_test=[],
            y_train=targets.astype(np.int64),
            y_test=[],
        )
    ...
    if pbar is not None:
        old_desc = pbar.desc
        pbar.set_description(model_fit_opt_desc)
...
def main_loop(args):
    ...
    state_dict_scaled_predictor = torch.load(args.scaled_predictor_state_dict, map_location=torch.device('cpu'))
    ...
    results = dict(
        ...
        opt_latent_points_t=[], # saves corresponding latent points
        opt_latent_points_b=[],
        opt_model_version=[],
        params=str(sys.argv),
    )
    ...
    with tqdm(
        ...
        for ret_idx in range(num_retrain):
            ...
            if args.lso_strategy == "opt":
                ...
                curr_model_path=result_dir / "retraining" / f"retrain_{samples_so_far}" / "checkpoints" / "last.ckpt"
                gp_dir.mkdir(parents=True)
                bo_data_file = gp_dir / "data.npz"
                x_new, y_new, z_query_t, z_query_b = latent_optimization(
                    args,
                    model,
                    scaled_predictor,
                    pixelsnail_bottom,
                    ...
                    pbar=pbar,
                    postfix=postfix,
                    curr_model_path=curr_model_path
                )
            else:
            ...
            # Save results
            results["opt_latent_points_t"] += [z for z in z_query_t]
            results["opt_latent_points_t"] += [z for z in z_query_b]
