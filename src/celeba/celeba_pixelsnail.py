...
class PixelSNAIL(nn.Module):
    def __init__(
        ...
        # load corresponding VQ-VAE2 model and freeze parameters
        self.vq_vae2 = CelebaVQVAE2.load_from_checkpoint(self.vqvae2_path)
        for p in self.vq_vae2.parameters():
    ...
    def training_step(self, batch, batch_idx):
        ...
        _, _, _, id_t, id_b = self.vq_vae2.encode(batch[0])
        #discrete_latents = discrete_latents.reshape(-1, 8, 8)
        train_loss = self.loss_function(id_t, id_b)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_progress_bar = True
        _, _, _, id_t, id_b = self.vq_vae2.encode(batch[0])
        val_loss = self.loss_function(id_t, id_b)
        self.logging_prefix = None
        self.log_progress_bar = False
        return val_loss
