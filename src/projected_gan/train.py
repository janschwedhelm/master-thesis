...
def launch_training(G, D, G_ema, num_steps, dataloader, outdir, num_gpus, batch_size, cond, batch_gpu,
    ...
    if restart_every > 0 and len(matching_dirs) > 0:  # expect unique desc, continue in this directory
    ...
    print(f'Training duration:   {num_steps} img')
    #print(f'Dataset path:        {c.training_set_kwargs.path}')
    #print(f'Dataset size:        {c.training_set_kwargs.max_size} images')
    print(f'Dataset resolution:  {256}')
    ...
    os.makedirs(run_dir, exist_ok=restart_every > 0)
...
def train(G, D, G_ema, num_steps, dataloader, outdir=None, num_gpus=1, batch_size=64, cond=False, batch_gpu=64,
    ...
    desc = f'fastgan-gpus{num_gpus:d}-batch{batch_size:d}'
