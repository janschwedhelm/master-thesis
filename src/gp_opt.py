""" Code to perform Bayesian Optimization with sparse GP or DNGO """
...
parser.add_argument(
    ...
    type=int,
    default=20,
    help="Number of optimization runs with different initial values"
)
parser.add_argument(
    "--n_samples",
    type=int,
...
def robust_multi_restart_optimizer(
    ...
    if opt_constraint_threshold is None:
        ...
        if opt_constraint_strategy == "gmm_fit":
...
def gp_opt(surrogate_file, data_file, save_file, logfile, n_samples, sample_distribution, num_pts_to_return, num_starts, method="L-BFGS-B",
           opt_constraint_threshold=None, opt_constraint_strategy="gmm_fit", n_gmm_components=None,
           sparse_out=True, pretrained_model_type="vae", bo_surrogate="GP"):
    ...
    # Choose other bounds
    if pretrained_model_type == "vae":
        opt_bounds = 3
        LOGGER.info(f"Using optimization bound of {opt_bounds}")
    elif pretrained_model_type == "rae":
        opt_bounds = np.percentile(np.abs(X_train), 99.9)  # To account for outliers
        LOGGER.info(f"Data bound of {opt_bounds} found")
        opt_bounds *= 1.1
        LOGGER.info(f"Using optimization bound of {opt_bounds}")
    else:
        raise NotImplementedError(pretrained_model_type)
...
if __name__ == "__main__":
    ...
    gp_opt(args.surrogate_file, args.data_file, args.save_file, args.logfile, args.n_samples, args.sample_distribution, args.n_out, args.n_starts, args.opt_method,
           ...
           args.n_gmm_components, args.sparse_out, args.pretrained_model_type, args.bo_surrogate)
